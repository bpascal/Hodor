...
public interface PubParamMapper {
    ...
    @Insert({
        ...
        "PARAM_VALUE, CHANNEL_FLAG)",
        "values (#{paramName,jdbcType=VARCHAR}, #{paramKey,jdbcType=VARCHAR}, ",
        "#{paramValue,jdbcType=VARCHAR}, #{channelFlag,jdbcType=VARCHAR})"
    })
    ...
    @ConstructorArgs({
        @Arg(column="PARAM_ID", javaType=Integer.class, jdbcType=JdbcType.INTEGER, id=true),
        ...
        @Arg(column="CHANNEL_FLAG", javaType=String.class, jdbcType=JdbcType.VARCHAR)
    })
    List<PubParam> selectByExample(PubParamExample example);

    @Select({
        "select",
        "PARAM_ID, PARAM_NAME, PARAM_KEY, PARAM_VALUE, CHANNEL_FLAG",
        "from t_pub_param",
        "where PARAM_ID = #{paramId,jdbcType=INTEGER}"
    })
    @ConstructorArgs({
    ...
    PubParam selectByPrimaryKey(Integer paramId);
