...
import com.codido.hodor.model.AccShopAcdtExample.Criteria;
import com.codido.hodor.model.AccShopAcdtExample.Criterion;
import com.codido.hodor.model.AccShopAcdtExample;
import java.util.List;
import java.util.Map;
import org.apache.ibatis.jdbc.SQL;

public class AccShopAcdtSqlProvider {

    public String countByExample(AccShopAcdtExample example) {
        SQL sql = new SQL();
        sql.SELECT("count(*)").FROM("t_acc_shop_acdt");
        applyWhere(sql, example, false);
        return sql.toString();
    }

    public String deleteByExample(AccShopAcdtExample example) {
        SQL sql = new SQL();
        sql.DELETE_FROM("t_acc_shop_acdt");
        applyWhere(sql, example, false);
        return sql.toString();
    }

    public String insertSelective(AccShopAcdt record) {
        SQL sql = new SQL();
        sql.INSERT_INTO("t_acc_shop_acdt");
        
        if (record.getShopAccountId() != null) {
            sql.VALUES("SHOP_ACCOUNT_ID", "#{shopAccountId,jdbcType=BIGINT}");
        }
    ...
    public String selectByExample(AccShopAcdtExample example) {
        SQL sql = new SQL();
        if (example != null && example.isDistinct()) {
            sql.SELECT_DISTINCT("SHOP_ACDT_ID");
        } else {
            sql.SELECT("SHOP_ACDT_ID");
        }
    ...
    public String updateByExampleSelective(Map<String, Object> parameter) {
        AccShopAcdt record = (AccShopAcdt) parameter.get("record");
        AccShopAcdtExample example = (AccShopAcdtExample) parameter.get("example");
        
        SQL sql = new SQL();
        sql.UPDATE("t_acc_shop_acdt");
        
        if (record.getShopAcdtId() != null) {
            sql.SET("SHOP_ACDT_ID = #{record.shopAcdtId,jdbcType=BIGINT}");
        }
    ...
    public String updateByPrimaryKeySelective(AccShopAcdt record) {
        SQL sql = new SQL();
        sql.UPDATE("t_acc_shop_acdt");
        
        if (record.getShopAccountId() != null) {
            sql.SET("SHOP_ACCOUNT_ID = #{shopAccountId,jdbcType=BIGINT}");
        }
        ...
        sql.WHERE("SHOP_ACDT_ID = #{shopAcdtId,jdbcType=BIGINT}");
        
        return sql.toString();
    }

    protected void applyWhere(SQL sql, AccShopAcdtExample example, boolean includeExamplePhrase) {
        if (example == null) {
            return;
        }
