 ...
 */
@Component
public class ZdjlScanHandler extends AbstractHandler {
    ...
    public WxMpXmlOutMessage handle(WxMpXmlMessage wxMessage, Map<String, Object> context, WxMpService wxMpService, WxSessionManager sessionManager) throws WxErrorException {
        ...
        UserVo userVo = userService.queryUserInfoByOpenId(openId, CommonConstans.COMMON_CHANNEL_ZDJL_MP);//获取用户信息
        if (userVo != null) {
            ...
            if (!JBUtil.isStrEmpty(shopInviteCode)) {
                ...
                if (handleUserShopRelaDto.isHandlerResult()) {
                    ...
                    responseResult = new TextBuilder().build("欢迎" + userVo.getUserName() + "使用「中大奖了」，你已经成功绑定彩店：" + shopVo.getShopName() + "，赶快去买买买吧，线下彩店是安全购彩的最大保障：实体网点，真实出票，合法合规，在你身边。", wxMessage, wxMpService);
                } else {
                    responseResult = new TextBuilder().build(handleUserShopRelaDto.getHandlerMsg(), wxMessage, wxMpService);
                }
                //默认给用户打player的tag
                userService.saveUserTags(Long.valueOf(userVo.getUserId()), CommonConstans.USER_TAG_ID_PLAYER, openId, CommonConstans.COMMON_CHANNEL_ZDJL_MP);
            } else {
